...
public static class Defaults
{
    public static string GetUserAgentClient()
    {
        ...
        if (currContext != null)
        {
            ...
            {
                UserAgent = currContext.Request.UserAgent;
                if ((!string.IsNullOrWhiteSpace(UserAgent)) && currContext.Request.Headers != null && currContext.Request.Headers["User-Agent"] != null)
                {
                    UserAgent = currContext.Request.Headers["User-Agent"].ToString();
                }
            }
        }
        return UserAgent;
    }
    public class CreV3_PageOption
    {
    ...
    public static string domain = "http://www.sendmsg.co.il";
    ...
    public static Dictionary<string, Defaults.CreV3_PageOption> friendlyMapID_creaditorV3 = new Dictionary<string, Defaults.CreV3_PageOption>();
    public static int getSiteID_FromURL(HttpRequest Request, HttpServerUtility Server, string[] domains, string host, string friendlyUrl, string[] friendlyUrlArray)
    {
        int _friendlySite = 0;
        ...
        return _friendlySite;
    }
    public static string toLandingV3(HttpRequest Request, string pageID)
    {
        ...
        return ("/LandingV3.aspx?pageID=" + pageID + DelCache);
    }

    public static string findLandingByNickName(string LandingV3friendlyUrl, int siteID)
    {
        LandingV3friendlyUrl = EncryptionHelper.Encrypt(LandingV3friendlyUrl);
        var _siteID = EncryptionHelper.Encrypt(siteID.ToString());
        ...
        try
        {
            request = WebRequest.Create("https://panel.sendmsg.co.il/Creaditor_V3.ashx");
            //request = WebRequest.Create("http://localhost:53055/Creaditor_V3.ashx");

            request.Method = "POST";
            request.Headers.Add("pass", "fdR$#RFWEds&^errt345EFergTY4t35G@#$@#$2133yt");

            postData = "LandingV3friendlyUrl=" + LandingV3friendlyUrl + "&siteID=" + _siteID;
            ...
            using (dataStream = request.GetRequestStream())
            {
                ...
                {
                    var a = (((HttpWebResponse)response).StatusDescription);
        ...
        {
            Defaults.ErrorWritingToDB(ref ex, "Defaults.cs");
        }
        return responseFromServer;
    }

    public static void insertStatistic(int siteID, int userID, int msgID)
    {
        Thread thread = new Thread(() => insertStatisticWebRequest(siteID, userID, msgID));
        thread.Start();
    }
    private static void insertStatisticWebRequest(int siteID, int userID, int msgID)
    {
        WebRequest request;
        string postData;
        byte[] byteArray;
        ...
        try
        {
            request = WebRequest.Create("https://panel.sendmsg.co.il/statistic.ashx");

            request.Method = "POST";
            postData = "MethodName=insertStatistic&siteID=" + siteID + "&userID=" + userID + "&msgID=" + msgID;
            byteArray = Encoding.UTF8.GetBytes(postData);
            // Set the ContentType property of the WebRequest.
            request.ContentType = "application/x-www-form-urlencoded";
            request.ContentLength = byteArray.Length;
    ...
    public static void ErrorWritingToDB(ref Exception ex, string PageSendr, string extraData = null)
    {
        LogIntoMySql(ex, PageSendr, extraData);
    }
    private static void LogIntoMySql(Exception ex, string PageSendr, string extraData)
    {
        string HeaderText = ex.Message;
        string Boby = ex.StackTrace;
        int lineWriteingError = 0;
        using (MySqlConnection conn = new MySqlConnection(Defaults.ConnStr))
        {
            try
            {
                conn.Open();
                string sql = string.Format("INSERT INTO ErrorLogs (ErrorLogsPage,ErrorLogsHeader,ErrorLogsBoby,ErrorLogsDate" + (lineWriteingError == 0 ? "" : ",lineWriteingError") + ")" +
                    "VALUES ('{0}','{1}','{2}',NOW()" + (lineWriteingError == 0 ? "" : ("," + lineWriteingError)) + ");", PageSendr.FixSqlInjection(), HeaderText.FixSqlInjection(), Boby.FixSqlInjection() + (extraData == null ? "" : extraData.FixSqlInjection()));
            ...
            catch (Exception ex2)
            {
                ...
                {
                    HttpContext.Current.Response.Write("Exception:" + ex2.Message + "<br/>");
                }
    ...
    public static void DelMsgFromTable(ref int TplID)
    {
        string tplID = TplID.ToString();
        lock (newsList)
        {
            var res = newsList.FirstOrDefault(f => f.OriginalTpl == tplID);
            if (res != null)
            {
                newsList.Remove(res);
            }
    ...
    public static string getDomainUrl(string domainUrl)
    {
        return domainUrl == "" ? domain : domainUrl;
    }
    ...
    public static SendMsgMinisite GetMiniste(int siteID, int landingID, MySqlConnection conn)
    {
        ...
        {
            //string sql = "SELECT langCode,FieldTextPass,isLocked,Password,Fields,Groups,landingContent,landingTitle,landingColor,landingMobileContent,landingColorMobile,landingBgImg,landingBgImgMobile,LandingBGImageOption,LandingBGImageOptionMobile, lp.siteID, ShowFacebook,showShare, landingLang, ShowFBComments, IndexInGoogle, tplDir, analyticsCode,ExternalCodeIntoBody,(SELECT contentNetworkActive FROM tblSites where tblSites.SIteID=lp.siteID) AS contentNetworkActive,PromoShowOnSide,PromoOnSideType,PromoShowOnBottom,PromoOnBottomType,PromoShowOnTop,PromoOnTopType,LandingPageWidth, (SELECT SiteStatus FROM tblSites where tblSites.SIteID=lp.siteID) AS SiteStatus FROM landingPages as lp LEFT JOIN landingpageslocked as lpl ON ( lpl.landingID = lp.landingID AND lpl.siteID = lp.siteID ) LEFT JOIN languages as lan ON lan.LangID=lp.landingLang  WHERE lp.siteID=" + siteID + " AND lp.landingID=" + landingID;
            string sql = "SELECT langCode,FieldTextPass,isLocked,Password,Fields,Groups,landingContent,IF(isUrlDomain=1 AND NOT (urlDomain is null or urlDomain=''),UrlDomain,IF(landingFriendly is NOT null AND landingFriendly<>'' , CONCAT('https://n.sendmsg.co.il/f' , CAST(lp.SiteID as CHAR CHARACTER SET utf8)  , '/' , landingFriendly) , CONCAT('https://n.sendmsg.co.il/Minisites.aspx?p=' , CAST(lp.landingID as CHAR CHARACTER SET utf8) , '-' , ((lp.SiteID + 3) * lp.landingID * 36) , '-' , CAST(lp.SiteID as CHAR CHARACTER SET utf8)))) AS LandingCanonical,landingTitle,landingColor,landingMobileContent,landingColorMobile,landingBgImg,landingBgImgMobile,LandingBGImageOption,LandingBGImageOptionMobile, lp.siteID, ShowFacebook,showShare, landingLang, ShowFBComments, IndexInGoogle, tplDir, analyticsCode,ExternalCodeIntoBody,contentNetworkActive,PromoShowOnSide,PromoOnSideType,PromoShowOnBottom,PromoOnBottomType,PromoShowOnTop,PromoOnTopType,LandingPageWidth, SiteStatus,panelUrl,landingUrl,websiteUrl,isCreaditor,OgTitle,OgDescription,OgImage FROM landingPages as lp LEFT JOIN landingpageslocked as lpl ON ( lpl.landingID = lp.landingID AND lpl.siteID = lp.siteID ) LEFT JOIN languages as lan ON lan.LangID=lp.landingLang LEFT JOIN tblSites as ts ON (lp.siteID = ts.siteID) LEFT JOIN SiteDomains as sd ON (ts.SiteDomainID = sd.SiteDomainID)  WHERE lp.siteID = " + siteID + " AND lp.landingID = " + landingID;

            MySqlCommand cmd = new MySqlCommand(sql, conn);
            MySqlDataReader dr = cmd.ExecuteReader();
            if (dr.Read())
            {
                string OgImage = dr["OgImage"].ToString();
                myMinisite.ogTitle = dr["OgTitle"].ToString();
                myMinisite.ogDescription = dr["OgDescription"].ToString();
                myMinisite.ogImage = (OgImage == "" ? "" : (Domain.getParamsByDomain(siteParamType.secureSitePath) + "/" + OgImage));
                myMinisite.landingLangCode = dr["langCode"].ToString();
                myMinisite.landingContent = dr["landingContent"].ToString();
                myMinisite.LandingCanonical = dr["LandingCanonical"].ToString();
                ...
                myMinisite.ExternalCodeIntoBody = dr["ExternalCodeIntoBody"].ToString();
                ...
                myMinisite.domain = dr["websiteUrl"].ToString();
                myMinisite.isCreaditor = (dr["isCreaditor"].ToString() == "1");
    ...
    public static SendMsgNewsletter GetNewsLetter(int siteID, int msgID, MySqlConnection conn)
    {
        ...
        {
            string sql = "SELECT MsgContent,MsgSubj, siteID, MsgLangID,RemoveCredits,OriginalTpl,Title,`Desc`,Images FROM SentMsgs Left Join sentmsgssocial ON sentmsgssocial.MsgID=SentMsgs.MsgID WHERE siteID=" + siteID + " AND SentMsgs.MsgID=" + msgID + " AND isMsgDel=0";
            MySqlCommand cmd = new MySqlCommand(sql, conn);
            MySqlDataReader dr = cmd.ExecuteReader();
            if (dr.Read())
            {
                ...
                myNewsletter.OriginalTpl = dr["OriginalTpl"].ToString();
                myNewsletter.SocialMsgNewsletter = new SocialMsgNewsletter(dr["Title"].ToString(), dr["Desc"].ToString(), dr["Images"].ToString());
                lock (newsList)
                {
    ...
    public static string[] activeStatusIds = { "1", "4", "5", "8", "9" };
    ...
    {
        return Domain.getSiteDomainParam(paramType, siteID);
    }
    public static long getCTR(int siteID, int courseID)
    {
        return ((siteID + 3) * (courseID + 5));
    }

    static Dictionary<int, string> langID_LangCode = new Dictionary<int, string>();

    public static string GetLangCodeFromLangID(int langID)
    {
        if (langID == 0)
        {
            langID = 1;
        ...
        if (langID_LangCode.ContainsKey(langID))
        {
            return langID_LangCode[langID];
        }
        else
        {
            using (MySqlConnection conn = new MySqlConnection(Defaults.ConnStr))
            {
                MySqlDataReader dr;
                conn.Open();
                MySqlCommand cmd = new MySqlCommand("SELECT LangCode FROM languages WHERE LangID=" + langID + ";", conn);
                dr = cmd.ExecuteReader();
                if (dr.Read())
                {
                    lock (langID_LangCode)
                    {
                        langID_LangCode.Add(langID, dr["LangCode"].ToString());
                    }
                }
                dr.Close();
            }
            //if the lanuguage will not be found, this will return an error, which is a desired behaviour.
            return langID_LangCode[langID];
        }
    }


    public static string GetLangCodeFromLangCode(string langCode)
    {

        var lang = langID_LangCode.FirstOrDefault(d => d.Value.ToLower().StartsWith(langCode.ToLower()));
        if (!string.IsNullOrWhiteSpace(lang.Value))
        {
            return lang.Value;
        }
        else
        {
            ...
            using (MySqlConnection conn = new MySqlConnection(Defaults.ConnStr))
            {
                MySqlDataReader dr;
                conn.Open();
                MySqlCommand cmd = new MySqlCommand("SELECT LangCode,LangID FROM languages Where langCode like '" + langCode.FixSqlInjection() + "%';", conn);
                dr = cmd.ExecuteReader();
                if (dr.Read())
                {
                    lock (langID_LangCode)
                    {
                        LangID = int.Parse(dr["LangID"].ToString());
                        if (!langID_LangCode.ContainsKey(LangID))
                        {
                            langID_LangCode.Add(LangID, dr["LangCode"].ToString());
                        }
            ...
            //if the lanuguage will not be found, this will return an error, which is a desired behaviour.
            string langText = "";
            if (!langID_LangCode.TryGetValue(LangID, out langText))
            {
                langText = "Eng";
            }
            return langText;
    ...
    public static string DomainName
    {
        get
        {
            return Domain.getParamsByDomain(siteParamType.DomainName);
        }
    }
    public static string DomainName2
    {
        get
        {
            return Domain.getParamsByDomain(siteParamType.DomainName2);
        }
    }
    public static string SitePath
    {
        get
        {
            return Domain.getParamsByDomain(siteParamType.sitePath);
        }
    }
    public static string SecureSitePath
    {
        get
        {
            return Domain.getParamsByDomain(siteParamType.secureSitePath);
        }
    }
    ...
    public static string MailPath
    {
        get
        {
            return Domain.getParamsByDomain(siteParamType.mailPath);
        }
    }
    public static string NewsLetterPath
    {
        get
        {
            return Domain.getParamsByDomain(siteParamType.newsLetterPath);
        }
    }
    public static string CourseDomain
    {
        get
        {
            return Domain.getParamsByDomain(siteParamType.courseDomain);
        }
    }
    public static string MultiMediaPath
    {
        get
        {
            return Domain.getParamsByDomain(siteParamType.multiMediaPath);
        }
    }
    public static string EndMailPath
    {
        get
        {
            return Domain.getParamsByDomain(siteParamType.endMailPath);
        }
    }
    public static string NPath
    {
        get
        {
            return Domain.getParamsByDomain(siteParamType.nPath);
        }
    }
    public static string HomeSite
    {
        get
        {
            return Domain.getParamsByDomain(siteParamType.homeSite);
        }
    }
    public static string CookieDomain
    {
        get
        {
            return Domain.getParamsByDomain(siteParamType.cookieDomain);
        }
    }

    public static string smtpPostfix
    {
        get
        {
            return Domain.getParamsByDomain(siteParamType.smtpPostfix);
        }
    }
    public static string smtpHost
    {
        get
        {
            return Domain.getParamsByDomain(siteParamType.smtpHost);
        }
    }
    public static string smtpUserName
    {
        get
        {
            return Domain.getParamsByDomain(siteParamType.smtpUserName);
        }
    }
    public static string smtpPass
    {
        get
        {
            return Domain.getParamsByDomain(siteParamType.smtpPass);
        }
    }
    public static string SupportMail
    {
        get
        {
            return Domain.getParamsByDomain(siteParamType.supportMail);
        }
    }
    ...
    public static string DefaultLang
    {
        get
        {
            return Domain.getParamsByDomain(siteParamType.DefaultLang);
        }
    }
    public static string TopLogo
    {
        get
        {
            return Domain.getParamsByDomain(siteParamType.TopLogo);
        }
    }
    public static string TopLogo2
    {
        get
        {
            return Domain.getParamsByDomain(siteParamType.TopLogo2);
        }
    }
    public static string TextUnderLogo
    {
        get
        {
            return Domain.getParamsByDomain(siteParamType.TextUnderLogo);
        }
    }
    public static string TextUnderLogo2
    {
        get
        {
            return Domain.getParamsByDomain(siteParamType.DomainName2);
    ...
    public static string LogoLink
    {
        get
        {
            return Domain.getParamsByDomain(siteParamType.LogoLink);
        }
    }

    public static string LogoLink2
    {
        get
        {
            return Domain.getParamsByDomain(siteParamType.LogoLink2);
        }
    }
    ...
    public static string BottomLogo
    {
        get
        {
            return Domain.getParamsByDomain(siteParamType.TextUnderBottomLogo);
        }
    }
    public static string BottomLogo2
    {
        get
        {
            return Domain.getParamsByDomain(siteParamType.BottomLogo2);
        }
    }
    public static string TextUnderBottomLogo
    {
        get
        {
            return Domain.getParamsByDomain(siteParamType.TextUnderBottomLogo);
    ...
    public static string TextUnderBottomLogo2
    {
        get
        {
            return Domain.getParamsByDomain(siteParamType.TextUnderBottomLogo2);
        }
    }
    public static string LoginPanelBg
    {
        get
        {
            return Domain.getParamsByDomain(siteParamType.LoginPanelBg);
        }
    }
    public static string CharImage
    {
        get
        {
            return Domain.getParamsByDomain(siteParamType.CharImage);
        }
    }
    public static string BgImg
    {
        get
        {
            return Domain.getParamsByDomain(siteParamType.BgImg);
        }
    }
    public static string InnerLogo
    {
        get
        {
            return Domain.getParamsByDomain(siteParamType.InnerLogo);
        }
    }
    public static string InnerLogo2
    {
        get
        {
            return Domain.getParamsByDomain(siteParamType.InnerLogo2);
        }
    }
    public static string FooterCredit
    {
        get
        {
            return Domain.getParamsByDomain(siteParamType.FooterCredit);
        }
    }
    public static string FooterCredit2
    {
        get
        {
            return Domain.getParamsByDomain(siteParamType.FooterCredit2);
        }
    }
    public static string HideHomeFooter
    {
        get
        {
            return Domain.getParamsByDomain(siteParamType.HideHomeFooter);
        }
    }
    public static string TransLanguages
    {
        get
        {
            return Domain.getParamsByDomain(siteParamType.TransLanguages);
        }
    }
    public static string LoginTitle
    {
        get
        {
            return Domain.getParamsByDomain(siteParamType.LoginTitle);
        }
    }
    public static string LoginTitle2
    {
        get
        {
            return Domain.getParamsByDomain(siteParamType.LoginTitle2);
        }
    }
    public static string LoginDescription
    {
        get
        {
            return Domain.getParamsByDomain(siteParamType.LoginDescription);
        }
    }
    public static string LoginDescription2
    {
        get
        {
            return Domain.getParamsByDomain(siteParamType.LoginDescription2);
        }
    }
    public static string InnerTitle
    {
        get
        {
            return Domain.getParamsByDomain(siteParamType.InnerTitle);
        }
    }
    public static string InnerTitle2
    {
        get
        {
            return Domain.getParamsByDomain(siteParamType.InnerTitle2);
        }
    }
    public static string InnerDescription
    {
        get
        {
            return Domain.getParamsByDomain(siteParamType.InnerDescription);
        }
    }
    public static string InnerDescription2
    {
        get
        {
            return Domain.getParamsByDomain(siteParamType.InnerDescription2);
        }
    }
    public static string LoginHeading
    {
        get
        {
            return Domain.getParamsByDomain(siteParamType.LoginHeading);
        }
    }
    public static string LoginHeading2
    {
        get
        {
            return Domain.getParamsByDomain(siteParamType.LoginHeading2);
        }
    }
    public static string LoginPopBg
    {
        get
        {
            return Domain.getParamsByDomain(siteParamType.LoginPopBg);
        }
    }
    public static string ExpiredMessage
    {
        get
        {
            return Domain.getParamsByDomain(siteParamType.ExpiredMessage);
        }
    }

    public static string ExpiredMessage2
    {
        get
        {
            return Domain.getParamsByDomain(siteParamType.ExpiredMessage2);
        }
    }
    ...
    {
        return GetPages(id, getDomainByUrl());
    }

    public static string GetPages(int id, string lang)
    {
        return GetPages(id, lang, getDomainByUrl());
    }

    public static string GetPages(int id, int domainID)
    {
        int langID = 2;
        if (!int.TryParse(HttpContext.Current.Session["Lang"].ToString(), out langID))
        {
            langID = 2;
        }
        return GetPages(id, Defaults.GetLangCodeFromLangID(langID), domainID);
    }
    public static string GetPages(int id, string lang, int domainID)
    {
        if (!langID_LangCode.ContainsValue(lang))
        {
            lang = "eng";
        }
        string pagecontent = "";
        using (MySqlConnection conn = new MySqlConnection(ConnStr))
        {
            ...
            MySqlCommand cmd = new MySqlCommand();
            cmd.Connection = conn;
            cmd.CommandText = "SELECT " + lang + " FROM tblPages WHERE DomainID=" + domainID + " AND pageNum=" + id;
            MySqlDataReader MyReader = cmd.ExecuteReader();

            if (MyReader.Read())
            {
                pagecontent = MyReader[lang].ToString();
            }
            MyReader.Close();
            conn.Close();
        }

        MatchCollection resCoustomField = Regex.Matches(pagecontent, @"\[\[.*?\]\]");
        if (resCoustomField != null && resCoustomField.Count > 0)
        {
            List<string> resKeys = resCoustomField.Cast<Match>().Select(d => d.Value.Replace("[", "").Replace("]", "")).ToList();
            siteParamType myEnumDomainParam;
            List<siteParamType?> resEnumDomainParam = resKeys.Select(d => Enum.TryParse(d, out myEnumDomainParam) ? ((siteParamType?)myEnumDomainParam) : null).Where(d => d != null).ToList();

            foreach (var item in resEnumDomainParam)
            {
                pagecontent = pagecontent.Replace("[[" + item.ToString() + "]]", Domain.getParamsByDomain(((siteParamType)item), domainID));
            }
        }
        return pagecontent;
    }
...
public class SendMsgMinisite
{
    ...
    public string LandingCanonical;
    ...
    public string ExternalCodeIntoBody;
...
public class MinisiteLocked
{
    ...
    public void lockPage(Panel LandingPageLocked, int landingID, int siteID, string landingLangCode, bool isMobile, HtmlGenericControl SecurityFooter, Literal titleSecurityHolder)
    {
        ...
        if (isMobile)
        {
            ...
            div.ID = "HolderFormTable";
            div.Controls.Add(table);
            LandingPageLocked.Controls.Add(div);

            Button b = new Button();

            var cvp = Defaults.getControlByID(table, b.GetType(), "CheckValidPass");
            ((Button)cvp).Attributes["style"] += "margin-bottom: 10px;";
            div.Attributes["style"] = "height: calc(100% - 216px);overflow: scroll;position: relative;";//(143px - 73px) = 216px

            LandingPageLocked.Attributes["style"] = "height: 100%;width: 100%;";
            table.Attributes["style"] += "padding:0% 5%;min-width: initial;transition: none;visibility:hidden";
            SecurityFooter.Attributes["style"] = "bottom: 0;background: #5b595a;height: 73px;overflow: hidden;";
            LandingPageLocked.Controls.Add(SecurityFooter);
            titleSecurityHolder.Text = "<div><img src=\"https://n.sendmsg.co.il/images/SecurityIconDark_" + (landingLangCode.ToLower() == "heb" ? "Heb" : "Eng") + ".png\" class=\"SecurityIcon\" ></div>";
            Literal l = new Literal();
            l.Text = "<script type=\"text/javascript\"> function vis(){ window.setTimeout(function(){  $('#FormTable').css({'visibility':'visible'}) },5)} function fixHeight(){if($('#FormTable').height() < $('#HolderFormTable').height()){$('#FormTable').addClass('fixHeight')}else{$('#FormTable').removeClass('fixHeight')}} fixHeight();vis();</script>";
            LandingPageLocked.Controls.Add(l);
        }
        else
        {
            Literal l = new Literal();
            l.Text = "<style>@media screen and (max-height: 735px) {.SecurityHolder {display: none;}}@media screen and (max-height: 700px) {.SecurityFooter {display: none;}}</style>";
            LandingPageLocked.Controls.Add(l);
        }
    }
...
public class SendMsgNewsletter
{
    ...
    public string OriginalTpl;
    public SocialMsgNewsletter SocialMsgNewsletter { get; set; }

}
public class SocialMsgNewsletter
{
    public string Title { get; private set; }
    public string Desc { get; private set; }
    public string Images { get; private set; }
    public SocialMsgNewsletter(string Title, string Desc, string Images)
    {
        this.Title = Title;
        this.Desc = Desc;
        this.Images = Images;
