...
<script RunAt="server">
    ...
    void Application_BeginRequest(Object sender, EventArgs e)
    {
        ...
        try
        {
            ...
            if (!currentUrl.EndsWith("/404.aspx"))
            {
                bool skip = false;
                ...
                else if (currentUrl.ToLower() == "/" || currentUrl.ToLower().Contains("aspxerrorpath"))
                {
                    ...
                    Response.Cookies.Add(new HttpCookie("data_Err", "aspxerrorpath"));
                ...
                else if (!currentUrl.ToLower().EndsWith("html"))
                {
                    ...
                    //host = "n.sendmsg.co.il";
                    //host = "comstar.minisite.ms";
                    ...
                    if (MatchLandingV3.Success)
                    {
                        ...
                        currentUrl = Defaults.toLandingV3(Request, Request.Url.Segments[Request.Url.Segments.Length - 1]);
                                        ...
                                        using (MySql.Data.MySqlClient.MySqlConnection con2 = new MySql.Data.MySqlClient.MySqlConnection(Defaults.ConnStr))
                                        {
                                            ...
                                            if (currentUrl != Server.UrlDecode(HttpContext.Current.Request.Url.PathAndQuery) && !skip)
                                            {
                                                if (currentUrl == "noFURL")
                                                {
                                                    try
                                                    {
                                                        Response.Redirect("/Minisites.aspx");
                                                    }
                                                    catch (Exception ex) { }
                                                }
                                                else
                    ...
                    else if (!MatchLandingV3.Success) // LandingV3  wite friendlyUrl
                    {
                        lock (Defaults.friendlyMapID_creaditorV3)
                        {
                            ...
                            {
                                shortLanding = shortLanding.Replace(("?" + Defaults.actionName + "=" + Defaults.rebuildName), "");
                                Defaults.friendlyMapID_creaditorV3.Remove(shortLanding);
                            }

                            if (shortLanding.Contains("?"))
                            {
                                // remove all paramerter from client
                                //domain.minisite.ms/sendmsg_survey-thankyou?returned=true&155=14/07/2022&479=2&157=%D7%92%D7%93%D7%9B%D7%92%D7%93%20%D7%93%D7%92%D7%9B%20%D7%93%D7%92&eMail=assaf@comstar.co.il&Status=Updated

                                //TO

                                //domain.minisite.ms/sendmsg_survey-thankyou

                                int s = shortLanding.IndexOf("?");
                                shortLanding = shortLanding.Remove(s);
                            }

                            Defaults.CreV3_PageOption CreV3_PageOption;


                            bool pageExist = Defaults.friendlyMapID_creaditorV3.TryGetValue(shortLanding, out CreV3_PageOption);
                            ...
                            if (!pageExist)
                            {
                                ...
                                if (MatchShortLanding.Success)
                                {
                                    ...
                                    if (siteID > 0)
                                    {
                                        string res = Defaults.findLandingByNickName(LandingV3friendlyUrl, siteID);
                                        if (!string.IsNullOrWhiteSpace(res))
                                        {
                                            if (res.Length < 400)
                                            {
                                                ...
                                                if (res.Length < 200)
                                                {
                                                    CreV3_PageOption = new System.Web.Script.Serialization.JavaScriptSerializer().Deserialize<Defaults.CreV3_PageOption>(res);
                                                    Defaults.friendlyMapID_creaditorV3.Add(shortLanding, CreV3_PageOption);
                                                }
                                            }
                                        }
                                        else
                                        {
                                            CreV3_PageOption = new Defaults.CreV3_PageOption();
                                            Defaults.friendlyMapID_creaditorV3.Add(shortLanding, CreV3_PageOption);
                                        }
                                    }
                                }
                            ...
                            if (CreV3_PageOption != null && !string.IsNullOrWhiteSpace(CreV3_PageOption.id))
                            {
                                currentUrl = Defaults.toLandingV3(Request, CreV3_PageOption.id);
                                Context.RewritePath("~/" + currentUrl);
                            }
                        }
                    }
        ...
        catch (Exception ex)
        {
            ...
            string data = ex.Message + "||" + ex.Source + "||" + ex.StackTrace;
            Response.Cookies.Add(new HttpCookie("data_Err", data));
