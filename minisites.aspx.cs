...
public partial class Minisites : System.Web.UI.Page
{
    ...
    {
        //var isPSL = Domain.DomainArr.Any(d => (!string.IsNullOrWhiteSpace(d.Value.Psl)) && Request.Url.Host.Contains(d.Value.Psl));
        ...
        urlRaw = Request.Url.Scheme + "://" + Request.Url.Host + Request.RawUrl;
        ...
        try
        {
            ...
            {
                FixStyleMethode();
                DragWindow_JS.Text = "<script src='https://" + Defaults.NPath + "/script/dragWindow.js'></script>";
    ...
    protected void Page_Load(object sender, EventArgs e)
    {
        ...
        favicon.Attributes["href"] = Domain.getParamsByDomain(siteParamType.secureSitePath, domainID) + "/" + Domain.getParamsByDomain(siteParamType.Favicon, domainID);
        ...
        // stop this because babab.i-send.co.il
        //string isGlobal = Request.QueryString["isGlobal"];
        //int domainIDByUrl = Domain.getDomainByUrl();
        //if (domainID != domainIDByUrl && isGlobal != "1" && siteID != 1000)
        //{
        //    Response.Redirect("404.aspx");

        //    //Response.Write(Defaults.NPath.ToLower().Replace("http://", "").Replace("https://", "") +"_"+ Request.Url.Host.ToLower());
        //    //Response.Write(HttpContext.Current.Session["SiteDomain"]);
        //}
        ...
        if (!string.IsNullOrEmpty(Request.QueryString["p"]))
        {
            ...
            if (parameters.Length == 3)
            {
                ...
                if (landingID == 0 || landingValidator == 0 || siteID == 0)
                {
                    Response.Redirect("404.aspx");
                }
        ...
        if (siteID != 0 && landingID != 0)
        {
            ...
            {
                if (!MyMinisiteRow.isCreaditor)
                {
                    Server.Transfer("minisites2.aspx" + (Request.QueryString.ToString() == "" ? "" : ("?" + Request.QueryString.ToString())));
                }
                if (Request.QueryString["lang"] == null)
                {
                    langId = MyMinisiteRow.landingLang;
                    langCode = Defaults.GetLangCodeFromLangID(langId);
                ...
                string psl = Domain.getParamsByDomain(siteParamType.psl, domainID);
                bool isPsl = (!string.IsNullOrWhiteSpace(psl));
                string NPath = Domain.getParamsByDomain(siteParamType.nPath, domainID);
                //string CookieDomain = Domain.getParamsByDomain(siteParamType.cookieDomain, domainID);
                ...
                if (Defaults.IsStatusActive(MyMinisiteRow.SiteStatus.ToString(), true))
                {
                    ...
                    msgContent = "<div dir=\"" + langDir + "\" style=\"direction:" + langDir + ";text-align:center; width:100%;font-family:Arial \"><div style=\"margin:auto; width:100%; font-family:Arial;\">";
                    //adds holder for "share on facebook" 

                    string commentStructure = "<!-- START External Code Into {0} -->{1}<!-- END External Code Into {0} -->";
                    AnaliticsCode.Text = (string.IsNullOrWhiteSpace(MyMinisiteRow.analyticsCode) ? "" : string.Format(commentStructure, "HEAD", MyMinisiteRow.analyticsCode));
                    ExternalCodeIntoBody.Text = (string.IsNullOrWhiteSpace(MyMinisiteRow.ExternalCodeIntoBody) ? "" : string.Format(commentStructure, "BODY", MyMinisiteRow.ExternalCodeIntoBody));
                    ...
                    //if (isPsl)
                    //{
                    //    NPath = NPath.Replace(CookieDomain, psl);
                    //}

                    LandingCanonical.Attributes["href"] = MyMinisiteRow.LandingCanonical.Replace("https://n.sendmsg.co.il", NPath);
                    if (!LandingCanonical.Attributes["href"].StartsWith("http"))
                    {
                        LandingCanonical.Attributes["href"] = "http://" + LandingCanonical.Attributes["href"];
                    }
                    if (isPsl)
                    {
                        LandingCanonical.Attributes["href"] = urlRaw;
                    }
                    ...
                    msgContent = msgContent.Replace("SendmsgForm.js\"", "SendmsgForm.js" + V + "\"");

                    msgContent += "</div></div></font>";
                    ...
                    int landingValidatorFromDB = 0;
                    {
                        ...
                        if (siteID != 0 && landingID != 0)
                        {
                            ...
                            msgContent = msgContent.Replace("id=\"showFacebook\">", "style=\"margin-left:10px; text-align:" + (langDir == "rtl" ? "right" : "left") + "; margin-right:10px; display:inline-block; text-align:" + (langDir == "rtl" ? "right" : " left") + "\"><a style=\"color:#919191;  text-decoration:none; font-weight:normal;font-size:14px; font-family:Arial; display:block; width:63; height:16px; text-align:" + (langDir == "rtl" ? "; right" : "; left") + "; vertical-align:middle; line-height:17px;  \" target=\"_blank\" href=\"https://www.facebook.com/sharer.php?u=" + Request.Url.Scheme + "://" + HttpUtility.UrlEncode(Defaults.NPath.Replace("https://", "") + "/Minisites.aspx?p=" + landingID + "-" + ctrl.ToString() + "-" + siteID.ToString() + "&lang=" + langId) + "\"><img src=\"" + Defaults.SecureSitePath + "/images/facebook_Icon.png\" alt=\"\" style=\"vertical-align:middle; border:none; margin-left:3px;\" />" + ShareFacebook + "</a> ");
                            msgContent = msgContent.Replace("id=\"FacebookShare\"", "target=\"_blank\" href=\"https://www.facebook.com/sharer.php?u=" + Request.Url.Scheme + "://" + HttpUtility.UrlEncode(Defaults.NPath.Replace("https://", "") + "/Minisites.aspx?p=" + landingID + "-" + ctrl.ToString() + "-" + siteID.ToString() + "&lang=" + langId) + "\"");
                            msgContent = msgContent.Replace("id=\"showForward\">", "style=\"display:inline-block;text-align:" + (langDir == "rtl" ? "right" : "left") + "margin-left:10px\"><a style=\"color:#919191; text-decoration:none; font-size:14px; font-family:Arial; display:inline-block; height:14px; vertical-align:middle; line-height:17px;  width:100px; margin-right:10px;\" href=\"mailto:?body=" + HttpUtility.UrlEncode(Defaults.NPath + "/Minisites.aspx?p=" + landingID.ToString() + "-" + ctrl.ToString() + "-" + siteID.ToString() + "&lang=" + langId) + "\"><img src=\"" + Defaults.NPath + "/images/forward_Icon.png\" style=\"vertical-align:middle; border:none; margin-left:3px;\" />" + sendFriend + "</a>");
                            ...
                            msgContent = Regex.Replace(msgContent, strRegex, "<div class=\"fb-like\" data-href=\"" + Request.Url.Scheme + "://" + Defaults.NPath.Replace("https://", "") + "/Minisites.aspx?p=" + landingID + "-" + ctrl.ToString() + "-" + siteID.ToString() + "&lang=" + langId + "\" data-layout=\"standard\" data-action=\"like\" data-show-faces=\"true\" data-share=\"false\"></div>");
                        }
                    }
                }
                else//if the site is not active
                {
                    ...
                    if (langId == 1)
                    {
                        ...
                        page += "<img src=\"" + Defaults.SecureSitePath + "/templateContentImages/images/TemplateGeneral/SendMsgLogoNoBg.png\" class=\"c-editable\">";
                        page += "</div>";
                    ...
                    else
                    {
                        ...
                        page += "<div class=\"logo c-ignore\" style=\"float: right; margin-right: 170px;\">";
                        page += "<img src=\"" + Defaults.SecureSitePath + "/userfiles/site4018/images/logo_sendmsg_en_png.JPG\" class=\"c-editable\" data-fullwidth=\"166\" data-fullheight=\"116\" width=\"166\" height=\"116\" style=\"width: 166px; height: 116px; min-width: initial;\">";
                        page += "</div>";
                ...
                // seo facebook & google start
                if (!string.IsNullOrWhiteSpace(MyMinisiteRow.ogDescription))
                {
                    PageOGDesc.Attributes["content"] = MyMinisiteRow.ogDescription;
                    PageDesc.Attributes["content"] = MyMinisiteRow.ogDescription;
                }
                if (!string.IsNullOrWhiteSpace(MyMinisiteRow.ogTitle))
                {
                    PageOGTitle.Attributes["content"] = MyMinisiteRow.ogTitle;
                }
                if (!string.IsNullOrWhiteSpace(MyMinisiteRow.ogImage))
                {
                    PageOGImage.Attributes["content"] = MyMinisiteRow.ogImage;
                }
                ogUrl.Attributes["content"] = LandingCanonical.Attributes["href"];
                if (isPsl)
                {
                    ogUrl.Attributes["content"] = urlRaw;
                    PageOGImage.Attributes["content"] = PageOGImage.Attributes["content"].Replace("https://n.sendmsg.co.il", (Request.Url.Scheme + "://" + Request.Url.Host));
                    StyleSheet.Attributes["href"]= StyleSheet.Attributes["href"].Replace("https://n.sendmsg.co.il", (Request.Url.Scheme + "://" + Request.Url.Host));
                }
                // seo facebook end
                        ...
                        for (int i = 3; i < 9; i++)
                        {
                            ...
                            if (showFlag)
                            {
                                ...
                                if (dr.Read())
                                {
                                    ...
                                    int.TryParse(dr["ContentAdUrl"].ToString(), out adLandingID);
                                    int.TryParse(dr["ContentAdID"].ToString(), out contentAdID);
                                    string hrefLink = Defaults.NPath + "/Minisites.aspx?p=" + adLandingID + "-" + ((adSiteID + 3) * adLandingID * 36) + "-" + adSiteID;
                                    string adTextDiv = CreateAdTextDiv(adType, dr["ContentAdAltText"].ToString(), dr["AdText"].ToString(), dr["ContentAdImage"].ToString(), hrefLink, adSiteID, siteID, contentAdID, landingID, i + 1);
                                    UpdateAdLiteral(i + 1, adTextDiv);
                                }
                                dr.Close();
            ...
            //if(Domain.DomainArr.Count(x => x.Value.DomainSiteID == siteID.ToString())==0)
            if (Request.QueryString["sid"] == null)
            {
                msgContent = msgContent.Replace(".regPopUp()", ".regPopUp({domainID:'" + domainID + "',sid:'" + siteID + "'})");
            }
            #endregion
        }
        else
        ...
        msgContent = msgContent.Replace("=\"/", "=\"https://panel.sendmsg.co.il/").Replace("url(/", "url(https://panel.sendmsg.co.il/").Replace("msgSubj", msgSubj).Replace("!~$#$~!", "'");
        msgContent = msgContent.Replace("~~!!#!!~~", "//");
        //msgContent = msgContent.Replace("&nbsp;", " ");
        ...
        string resText = "";
        Dictionary<string, string> wordByDomain = new Dictionary<string, string>();
        string key = "";
        foreach (siteParamType item in (siteParamType[])Enum.GetValues(typeof(siteParamType)))
        {
            key = "[~[" + item + "]~]";
            if (msgContent.Contains(key))
            {
                resText = Domain.getParamsByDomain(item, domainID);
                if (!wordByDomain.ContainsKey(key))
                {
                    wordByDomain.Add(key, resText);
                }
                else
                {
                    wordByDomain[key] = resText;
                }
            }
        }
        if (wordByDomain.Count > 0)
        {
            msgContent = msgContent.ReplaceMultiWord(wordByDomain, true);
        }
        string newcontact = SetNewContact(msgContent.Replace("'/AddUserFromSite.aspx'", "'" + Defaults.SecureSitePath + "/AddUserFromSite.aspx'"));


        if (siteID == 1000 && domainID > 0)
        {
            int tmpInt = 0;
            foreach (siteParamType suit in (siteParamType[])Enum.GetValues(typeof(siteParamType)))
            {
                var valThisDomain = Domain.getParamsByDomain(suit, domainID);

                var valBaseDomain = Domain.getParamsByDomain(suit, 0);

                if (!int.TryParse(valBaseDomain, out tmpInt))
                {
                    newcontact = newcontact.Replace(valBaseDomain, valThisDomain);
                }
            }
        }
        //new font plugin
        var fonts = new FontsPManager();
        fonts.AddFontsToPage(newcontact, FontsPlacement);


        //land page content as been put on page 
        ...
        if (siteID == Defaults.baseSite && Request.QueryString["Copyright"] == false.ToString().ToLower())
        {
        ...
        //if (!IsPostBack)
        //{
        MyMinisiteRow.PageLocked.lockPage(LandingPageLocked, MyMinisiteRow.landingID, MyMinisiteRow.siteID, MyMinisiteRow.landingLangCode, isMobile, SecurityFooter, titleSecurity);
        //}
    ...
    {
        if (MyMinisiteRow != null && MyMinisiteRow.isPageLocked && (Session["barCodePage" + MyMinisiteRow.landingID.ToString()] == null ? true : (MyMinisiteRow.PageLocked.barCodePage != Session["barCodePage" + MyMinisiteRow.landingID.ToString()].ToString())))
        {
            landingTemplate.Text = "";
            if (Defaults.background.Count == 0)
            {
                lock (Defaults.background)
                {

                    List<string> filesTemp = Directory.GetFiles(Server.MapPath("images/background"), "*.jpg").ToList();
                    filesTemp.ForEach(f => Defaults.background.Add(f.Replace(Server.MapPath(""), "").Replace("\\", "/")));
                }
            }

            Random rnd = new Random();
            int r = rnd.Next(Defaults.background.Count);
            string url = Request.Url.ToString();

            PageBody.Attributes["style"] += "background:url('" + (isMobile ? "none" : (url.Remove(url.LastIndexOf("/")) + Defaults.background[r])) + "');background-size: cover;";
            if (Defaults.DefaultLang == Defaults.GetLangCodeFromLangID(langId))
            {
                SecurityFooterImg.Src = Defaults.SecureSitePath + "/" + Defaults.TopLogo;
                SecurityIcon.Src = Defaults.NPath + "/images/SecurityIcon_Heb.png";
            }
            else
            {
                SecurityFooterImg.Src = Defaults.SecureSitePath + "/" + Defaults.TopLogo2;
                SecurityIcon.Src = Defaults.NPath + "/images/SecurityIcon_Eng.png";
            }
            AccessRequired.Text = Languages2.MyText("minisites_Access required identification_הכניסה לדף למורשים בלבד", MyMinisiteRow.landingLangCode);
            titleSecurity.Text += Languages2.MyText("minisites_The Page is secure, Please identify yourself_מלא את פרטיך לגישה לדף המאובטח", MyMinisiteRow.landingLangCode);
            PageCreated.Text = Languages2.MyText("minisites_Page created by a secure mail system_דף מאובטח ונבנה ע\"י מערכת דיוור", MyMinisiteRow.landingLangCode);
            SecurityFooterLink.HRef = Defaults.HomeSite;
            BottomAdsHeight.Visible = false;
        }
        else
        {
            LandingPageLockedHolder.Visible = false;
            BottomAdsHeight.Visible = true;

        }
        if (MyMinisiteRow != null && (Request.QueryString["lang"] == null || Request.QueryString["lang"] == "1") && (MyMinisiteRow.landingLang == 1 || MyMinisiteRow.landingLangCode.ToLower() == "heb"))
        {
            accessPlug.Text = "<script type=\"text/javascript\" src=\"https://app.sendmsg.co.il/getAccessPlug.ashx" + V + "\"></script>";
            //accessPlug.Text = "<script type=\"text/javascript\" src=\"http://localhost:63695/getAccessPlug.ashx"+ V + "\"></script>";
        }
    ...
    protected string CreateAdTextDiv(int adType, string contentAdAltText, string adText, string contentAdImage, string hrefLink, int adSiteID, int publisherSiteID, int ContentAdID, int currentLandingID, int adPosition)
    {
        ...
        {
            divAdHolder += "<img " + ("src=\"" + contentAdImage.ToLower() + "\")").Replace("src=\"uploads", "src=\"" + Defaults.SecureSitePath + "/Uploads").Replace("src=\"/uploads", "src=\"" + Defaults.SecureSitePath + "/Uploads").Replace("src=\"userfiles", "src=\"" + Defaults.SecureSitePath + "/userfiles") + " alt=\"" + contentAdAltText + "\" class=\"contentAdImageClass\" style=\"border:none;margin:0 5px 5px 5px;" + imgStyle + floatStyle + "\" />";
        }
            ...
            if (AdInfoArr.Length == 6)
            {
                ...
                if (int.TryParse(AdInfoArr[0], out publisherSiteID) && int.TryParse(AdInfoArr[1], out adSiteID) && int.TryParse(AdInfoArr[2], out ContentAdID) && int.TryParse(AdInfoArr[3], out currentLandingID))
                {
                    if (System.Web.Security.FormsAuthentication.HashPasswordForStoringInConfigFile(((ContentAdID + adSiteID) * 14 - currentLandingID).ToString(), "md5") == AdInfoArr[4])
                    {
                        ...
                        pageView2 tempPageView = new pageView2();
                        lock (PageViewsList)
                        {
